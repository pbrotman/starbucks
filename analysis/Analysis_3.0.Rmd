...
tbl_starbucks_per_county <- st_join(pts_starbucks, shape_county) %>% 
  group_by(AFFGEOID, STATEFP, COUNTYFP, NAME, `State/Province`) %>% 
  summarise(n = n()) %>% 
  arrange(desc(n))
```
...
### Combine census and Starbucks data
